...
public class FirebaseStoreServiceImpl implements FirebaseStoreService {
    ...
    return Completable.create(
        emitter -> {
          if (storeId.isBlank()) {
            emitter.onError(
                new FirebaseFirestoreException(
                    "Store ID cannot be empty", FirebaseFirestoreException.Code.INVALID_ARGUMENT));
            return;
          }
   ...
   *
   * <p>Parses the response data from Firebase Functions, converts it to a list of StoreDto objects
   * using GSON, and emits the result via the Single emitter. Handles potential parsing errors and
   * ensures the returned list is never null.
   *
  ...
  private static void onGetStoreSuccess(
      SingleEmitter<StoreFirebaseObject> emitter, @NonNull DocumentSnapshot documentSnapshot) {
    if (documentSnapshot.exists()) {
      StoreFirebaseObject storeFirebaseObject = toStore(documentSnapshot);
      if (storeFirebaseObject == null) {
        emitter.onError(
            new FirebaseFirestoreException(
                "Failed to parse store data", FirebaseFirestoreException.Code.INTERNAL));
      }
      emitter.onSuccess(storeFirebaseObject);
    } else {
      emitter.onError(
  ...
  @Nullable private static StoreFirebaseObject toStore(@NonNull DocumentSnapshot documentSnapshot) {
    ...
    StoreFirebaseObject storeFirebaseObject = documentSnapshot.toObject(StoreFirebaseObject.class);
