...
import com.optlab.banhangso.data.local.resource.NetworkBoundResource;
...
//                        .getAllStores(); // For testing purposes, fetch all stores
            ...
            @Override
            protected List<StoreEntity> mapNetworkResponseToDatabase(List<StoreDto> items) {
                return items.stream().map(StoreDto::toEntity).collect(Collectors.toList());
            }
