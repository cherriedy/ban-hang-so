...
constructor(
  ...
  override fun getBrands(): Flowable<PagingData<Brand>> =
    Pager(pagingConfig) { BrandPagingSource(preferencesRepository, brandService) }
      .flowable
  ...
  override fun searchBrands(query: String): Flowable<PagingData<Brand>> =
    Pager(pagingConfig) { BrandSearchPagingSource(preferencesRepository, brandService, query) }
      .flowable
      .map { pagingData -> pagingData.map(BrandFirebaseObjectMapper::toDomain) }
