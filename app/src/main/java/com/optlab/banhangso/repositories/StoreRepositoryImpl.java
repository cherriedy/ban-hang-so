...
import com.optlab.banhangso.models.remote.mapper.RoleStoreFirebaseObjectMapper;
import com.optlab.banhangso.models.remote.mapper.StoreFirebaseObjectMapper;
...
public class StoreRepositoryImpl implements StoreRepository {
    ...
    private final RenderStoreService renderStoreService;
                        ...
                        responseObject -> {
                            if (responseObject.isSuccess()) {
                                ...
                                List<RoleStore> roleStores =
                                        RoleStoreFirebaseObjectMapper.toDomains(
                                                roleStoreFirebaseObjects);
                                Timber.d(
                                        "Fetched %d stores for user %s", roleStores.size(), userId);
                                return Single.just(new Result.Success<>(roleStores));
                            } else {
                                Throwable throwable =
                                        new ApiResponseException(
                                                responseObject.message(), responseObject.code());
                                return Single.just(
                                        new Result.Failure<List<RoleStore>>(
