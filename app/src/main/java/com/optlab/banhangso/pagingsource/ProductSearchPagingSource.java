...
public class ProductSearchPagingSource extends RxPagingSource<Integer, ProductFirebaseObject> {
    ...
    public ProductSearchPagingSource(
            @NonNull String query, RenderProductService renderProductService) {
        this.query = query;
    ...
    private LoadResult<Integer, ProductFirebaseObject> mapToResult(
            RenderResponseObject<ProductResponseObject.ProductCollection>
                    productCollectionRenderResponseObject) {
        if (productCollectionRenderResponseObject.isError()) {
            Throwable throwable =
                    new ApiResponseException(
                            productCollectionRenderResponseObject.message(),
                            productCollectionRenderResponseObject.code());
            return new LoadResult.Error<>(throwable);
        } else {
            ProductResponseObject.ProductCollection productCollection =
                    productCollectionRenderResponseObject.data();
    ...
    public Integer getRefreshKey(@NonNull PagingState<Integer, ProductFirebaseObject> pagingState) {
        ...
        LoadResult.Page<Integer, ProductFirebaseObject> anchorPage = pagingState.closestPageToPosition(anchorPosition);
        if (anchorPage == null) {
            return null;
        }
