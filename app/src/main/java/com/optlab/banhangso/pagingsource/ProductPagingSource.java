...
public class ProductPagingSource extends RxPagingSource<Integer, ProductFirebaseObject> {
   ...
   */
  @Inject
  public ProductPagingSource(ProductService productService) {
   ...
   *
   * @param responseObject The response from the API containing product data
  ...
  private LoadResult<Integer, ProductFirebaseObject> toLoadResult(
      @NonNull ResponseObject<ProductResponseObject.ProductCollection> responseObject) {
    if (responseObject.isError() || responseObject.isFailure()) {
      Timber.e("Error loading products: %s", responseObject.message());
    ...
    ProductResponseObject.ProductCollection productCollection = responseObject.data();
